...
const LoadReports = ({ onBack, user, userProfile, deductions }: LoadReportsProps) => {
  ...
  const handleResetFilters = () => {
    resetFilters();
    setFilters({ fromDate: '', toDate: '', locationFrom: '', locationTo: '' });
    setIsFiltered(false);
  };

  const displayedLoads = isFiltered ? filteredLoads : currentWeekLoads;

  const totalGrossPay = displayedLoads.reduce((total, load) => total + (load.rate || 0), 0);
  const totalDriverPay = displayedLoads.reduce((total, load) => total + (load.driverPay || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-background brutal-grid p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <LoadSummaryCards
          currentWeekLoads={displayedLoads}
        ...
        {displayedLoads.length > 0 ? (
          <div className="space-y-4">
            <h3 className="brutal-text text-xl text-foreground">
              {isFiltered ? 'FILTERED_LOADS' : 'WEEK_LOADS'} ({displayedLoads.length})
            </h3>
          ...
          <div className="brutal-border bg-muted p-8 brutal-shadow text-center">
            ...
            <p className="brutal-mono text-sm text-muted-foreground">
              {isFiltered ? 'MATCHING_FILTERS' : 'FOR_THIS_WEEK'}
            </p>
          </div>
        )}
